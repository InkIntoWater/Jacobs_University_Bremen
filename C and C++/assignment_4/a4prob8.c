...
//declaring function to print the elements under the secondary diagonal.
void under_sec_diag (int array_2d[30][30], int n){
    for (int r=0; r<n; r++){
        for (int c=0; c<n; c++){
            if ((c+r)>(n-1)){
                printf("%d ", array_2d[r][c]);
...
int main(){
    ...
    the elements under the secondary diagonal.*/
    printf("The entered matrix is:\n");
    print_2d(array_2d, n);
    printf("Under the secondary diagonal:\n");
    under_sec_diag(array_2d, n);
    return 0;
}